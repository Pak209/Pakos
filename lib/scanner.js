...
function scanRepo(name, repoPath) {
  ...
  const commits = [];
  if (!project.isGit) return { project, commits };
  ...
  try {
    ...
    project.branchCount = git(repoPath, ['branch', '--list'])
      .split('\n').filter(Boolean).length;

    // %x1f = unit separator: safe against | or tabs inside subjects.
    const log = git(repoPath, ['log', '-8', '--format=%h%x1f%s%x1f%an%x1f%cI']);
...
function scan() {
  ...
  for (const entry of entries) {
    ...
    const { project, commits: repoCommits } = scanRepo(entry.name, repoPath);
