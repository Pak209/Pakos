...
function replaceScan({ projects, tasks, commits, scannedAt, durationMs }) {
  db.exec('BEGIN');
  try {
    db.exec('DELETE FROM projects; DELETE FROM tasks; DELETE FROM commits;');
    ...
    const insTask = db.prepare(
      'INSERT INTO tasks (project, title, status, source_file, line) VALUES (?,?,?,?,?)');
    ...
    for (const t of tasks) insTask.run(t.project, t.title, t.status, t.sourceFile, t.line);
...
module.exports = { db, replaceScan, getState, DB_PATH };
