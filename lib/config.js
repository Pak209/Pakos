...
const DEFAULTS = Object.freeze({
  ...
  usage: {},
  // Portfolio health dimension weights (docs/INTELLIGENCE.md §4).
  health: {
    weights: { momentum: 0.25, hygiene: 0.2, recovery: 0.2, debt: 0.15, direction: 0.2 },
  },
...
function getConfig() {
  ...
  config.health = { ...structuredClone(DEFAULTS.health), ...config.health };
  config.health.weights = { ...structuredClone(DEFAULTS.health.weights), ...config.health.weights };
  if (!config.authToken) {
