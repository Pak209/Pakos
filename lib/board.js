...
const { headingStatus, cleanTitle } = require('./tasks');
...
function moveMission(root, { project, sourceFile, line, title, toStatus }) {
  ...
  const m = current?.match(/^\s*[-*+]\s+(?:\[[ xX]\]\s+)?(.+)/);
  if (!m || cleanTitle(m[1]) !== cleanTitle(String(title ?? ''))) {
    throw new BoardError(409, 'board changed since last scan — rescan and retry');
  }

  lines.splice(idx, 1);
  insertUnderStatus(lines, toStatus, missionLineFor(toStatus, cleanTitle(m[1])));
  writeAtomic(file, lines.join('\n'));
  return { project, title: cleanTitle(m[1]), status: toStatus };
}
