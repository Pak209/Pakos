...
// Board spec v1: an optional trailing "@owner" assigns a crew member
// ("- [ ] Fix the scanner @codex"). Only at the end of the line, so
// emails and mid-sentence @mentions never become owners.
function splitOwner(raw) {
  const m = raw.match(/^(.*?)\s+@([a-z][\w-]{0,31})\s*$/i);
  return m ? { text: m[1], owner: m[2].toLowerCase() } : { text: raw, owner: null };
}
...
function parseFile(project, filePath, sourceLabel, { requireStatusHeading }) {
  ...
  text.split('\n').forEach((line, i) => {
    ...
    if (checkbox && effectiveStatus) {
      ...
      const { text, owner } = splitOwner(checkbox[2]);
      const title = cleanTitle(text);
      if (title) tasks.push({ project, title, owner, status: done ? 'done' : effectiveStatus, sourceFile: sourceLabel, line: i + 1 });
    } else if (bullet && sectionStatus) {
      // plain bullets only count under an explicit status heading
      const { text, owner } = splitOwner(bullet[1]);
      const title = cleanTitle(text);
      if (title) tasks.push({ project, title, owner, status: sectionStatus, sourceFile: sourceLabel, line: i + 1 });
    }
  });
  return tasks;
}
...
module.exports = { parseProjectTasks, headingStatus, cleanTitle, splitOwner };
